...
//       imubuf_[imubuf_head_].data.a[0] = t0;
...
void setup() {
  ...
  xTaskCreatePinnedToCore(vTaskGetData, "vTaskGetData", 1024, NULL, 24, NULL, 0);
...
#ifndef USE_SERIAL
//          imubuf_[imubuf_tail_].data.m[0] = micros() * 1e-6;
...
//        imubuf_[imubuf_tail_].data.m[0] = micros() * 1e-6;
    ...
    if (file_) {
      ...
      while (file_.position() < file_.size()) {
        ...
        char txt[txtlen];
        snprintf(txt, txtlen, "%d, %3.6f, %3.6f, %7.4f, %7.4f, %9.5f, %9.5f, %9.5f, %3.5f, %4.0f, %4.0f\n",
